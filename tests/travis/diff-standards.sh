...
./vendor/bin/phpcs --standard=./vendor/vanilla/standards/code-sniffer/Vanilla --report=json $(git diff --name-only --diff-filter=dr $DIFF_BRANCH -- '*.php') > $PHPCS_DIFF_FILENAME
